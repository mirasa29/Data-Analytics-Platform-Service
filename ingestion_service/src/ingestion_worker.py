...
class IngestionWorker:
    ...
    def _extract_from_rdbms_and_produce(self, last_extracted_value) -> str:
        ...
        try:
            ...
            if produced_count > 0:
                self.last_ingestion_time = datetime.now()
                logging.info(f"Finished extraction. Produced {produced_count} messages to Kafka topic.")
    ...
    def _extract_from_file_and_produce(self) -> None:
        try:
            ...
            if file_path.endswith('.json'):
                with open(file_path, 'r') as file:
                    data = json.load(file)
                    # logging.info(f"Loaded JSON file {data} - making sure full data is read.")
            elif file_path.endswith('.csv'):
                with open(file_path, 'r') as file:
                    reader = csv.DictReader(file, delimiter=self.config.producer.source_config.delimiter)
                    data = list(reader)
            else:
                raise IngestionWorkerError(f"Unsupported file type for ingestion: {file_path}", "Dataset")

            for record in data:
                # logging.info(f"Processing record: {record} - make sure it iterates through all records.")
                try:
                    pk_values = {k: record.get(k) for k in self.config.dataset.primary_keys}
                    self.producer.produce(
                        self.config.kafka_config.topic,
            ...
            self.last_ingestion_time = datetime.now()
            logging.info(f"File ingestion finished. Messages produced: {len(data)}")

        except FileNotFoundError:
            raise IngestionWorkerError(f"Source file not found at: {self.config.producer.source_config.file_path}",
                                       "Dataset")
    ...
    def _consume_and_load(self) -> int:
        try:
            poll_timeout = min(10000.0 / 10000.0, self.config.s3_landing_config.flush_interval_seconds / 2.0)
            msg = self.consumer.poll(timeout=poll_timeout)
            if not msg:
                if (datetime.now() - self.last_flush_time).total_seconds() >= self.config.s3_landing_config.flush_interval_seconds:
    ...
    def run(self) -> None:
        ...
        if source_type == 'rdbms':
            ...
            self.last_extracted_value = self.state_manager.load_state().get('last_watermark_value',
                                                                            self.config.dataset.incremental_load.initial_watermark_value)
            if isinstance(self.last_extracted_value,
                          str) and self.config.dataset.incremental_load.watermark_type == 'timestamp':
                self.last_extracted_value = datetime.fromisoformat(self.last_extracted_value)
            ...
            try:
                ...
                max_empty_polls = 5  # threshold
                while True:
                    if (datetime.now() - self.last_check_time) >= self.poll_interval:
                        self._get_db_connection()
                        new_watermark = self._extract_from_rdbms_and_produce(self.last_extracted_value)

                        if new_watermark and new_watermark != self.last_extracted_value:
                            self.state_manager.save_state({'last_watermark_value': new_watermark})
                            self.last_extracted_value = new_watermark
                            logging.info(f"New watermark saved: {new_watermark}")
                            consecutive_empty_polls = 0  # reset counter
                        else:
                            consecutive_empty_polls += 1
                            logging.info(f"No new data found. Consecutive empty polls: {consecutive_empty_polls}")
                            if consecutive_empty_polls >= max_empty_polls:
                                logging.info("No new data for multiple intervals. Shutting down worker gracefully.")
                                break
                ...
                if self.db_conn:
                    self.db_conn.close()
                logging.info("Resource clean up completed - thread terminating")
        elif source_type == 'flat_file':
            logging.info("Starting one-time file ingestion and consumption.")
            self._get_kafka_producer()
            self._get_minio_client()

            self._extract_from_file_and_produce()
            self.producer.flush()
